...
if __name__ == "__main__":
    ...
    for plot in XSEC_TO_MAKE:
        ...
        # I don't trust DrawDataMCRatio Anymore. The uncertainty was calculated in a way I don't agree (or I didn't understand).
        ...
        plotter = lambda mnvplotter,data_hist, mc_hist, *mc_ints : partial(PlotTools.MakeSignalDecomposePlot,color=colors,title=titles,ratio=True)(data_hist,PlotUtils.MnvH1D(mc_hist.GetCVHistoWithStatError()),mc_ints)
        PlotTools.MakeGridPlot(PlotTools.Make2DSlice,plotter,[xsec,mc_xsec,*sig_dep],draw_seperate_legend=True)
        PlotTools.Print(AnalysisConfig.PlotPath(PLOT_SETTINGS[plot]["name"],playlist,"xsec_sigdep_ratio"))
